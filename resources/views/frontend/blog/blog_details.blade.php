...
<!--End Page Title-->
        ...
        <section class="sidebar-page-container blog-details sec-pad-2">
           ...
           <figure class="author-thumb">
            <img src="{{ (!empty($blog->user->photo)) ?
url('uploads/admin_images/'.$blog->user->photo) : url('uploads/admin_images/no_image.jpg') }}" alt="profile">
    </figure>

<span>{{ $blog['user']['name']}}</span>

                                                <h5><a href="{{ url('blog/details/'.$blog->post_slug) }}">{{ $blog['user']['name']}}</a></h5>
                                            </li>
...
$comment = App\Models\Comment::where('post_id', $blog->id)->where('parent_id', null)->limit(5)->get();
        ...
        url('uploads/user_images/'.$com->user->photo) : url('uploads/user_images/no_image.jpg') }}" alt="profile">
                                        ...
                                        <div class="comment-inner">
                                            ...
                                            <div class="text">
                                                <p>{{$com->subject }}</p>
                                                <p>{{$com->message }}</p>
                                                <a href="{{ url('blog/details/'.$blog->post_slug) }}"><i class="fas fa-share"></i>Reply</a>
                                            </div>
                                        </div>
                                    </div>
                                    @php
                                    ...
                                    <div class="comment replay-comment">
                                        <figure class="thumb-box">
                                            <img src="{{ url('uploads/admin_images/admin.jpg') }}" alt="">
                                        </figure>
                                        <div class="comment-inner">
                                            <div class="comment-info clearfix">
                                                <h5>{{ $rep->subject }}</h5>
...
{{-- ///getting alll data by blog post id --}}
                                ...
                                @else
                                <P><b>To Comment please login <a href="{{route('login') }}"> Login Here</a></b></P>
                                @endauth
                            </div>
                        </div>
                    </div>
                ...
                <li><a href="{{ url('blog/cat/list/'.$cat->id) }}">{{$cat->category_name}}<span>({{ count($post) }})</span></a></li>
                            ...
                            <div class="sidebar-widget post-widget">
                                ...
                                <div class="post-inner">
                                    ...
                                    <div class="post">
                                        <figure class="post-thumb"><a href="{{ url('blog/details/'.$blog->post_slug) }}"><img src="{{asset($post->post_image)}}" alt=""></a></figure>
                                        <h5><a href="{{ url('blog/details/'.$blog->post_slug) }}">{{$post->post_title  }}</a></h5>
